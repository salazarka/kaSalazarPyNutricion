...
import { StyleSheet, Text, View, ListView, Alert, Button, TouchableOpacity, Image, TouchableHighlight } from 'react-native';
...
export class Home extends React.Component {
    ...
    lookingChild(idCliente) {
        itemRef = this.getRef().child('client');
        clientChild = '';
        itemRef.on('value', (snap) => {
            snap.forEach((child) => {
                if ((child.key == idCliente)) {
                    clientChild = child.val();
                }
            });

        });
        this.props.navigation.navigate('ProfileScreen', { itemId: clientChild })
    }
        ...
        return ( <
            ...
            onPress = {
                () => Alert.alert(
                    ...
                    item.note, [
                        { text: 'Cancel' },
                        { text: 'Make a plan', onPress: () => { this.makingPlan(item.title, itemId) } }
                    ], { cancelable: false })
            } > { item.title }
    ...
    render() {
        return (
            ...
            color = "#aa6d71"
            onPress = { // {this.lookingChild(idClienteA)}this.props.navigation.navigate('ProfileScreen', { itemId: idClienteA })
                () => { this.lookingChild(idClienteA) }
            }
            / >
